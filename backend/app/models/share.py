...
class Share(Base, CreatedAtMixin):
    ...
    # Required fields
    type: Mapped[ShareType] = mapped_column(SQLEnum(ShareType), nullable=False)
    source_type: Mapped[ShareSourceType] = mapped_column(SQLEnum(ShareSourceType), nullable=False, name="source_type")
    ...
    channel: Mapped[ShareChannel] = mapped_column(SQLEnum(ShareChannel), nullable=False, default=ShareChannel.internal)
    status: Mapped[ShareStatus] = mapped_column(SQLEnum(ShareStatus), nullable=False, default=ShareStatus.pending)
