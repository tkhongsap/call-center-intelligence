...
def create_app() -> FastAPI:
    ...
    async def validation_exception_handler(request, exc: ValidationError):
        logger.warning(f"Validation error occurred: {str(exc)} at path: {request.url.path}")
        return JSONResponse(
            status_code=422,
            content={
                "error": {
    ...
    async def authentication_exception_handler(request, exc: AuthenticationError):
        logger.warning(f"Authentication error occurred: {str(exc)} at path: {request.url.path}")
        return JSONResponse(
            status_code=401,
            content={
                "error": {
    ...
    async def conflict_exception_handler(request, exc: ConflictError):
        logger.warning(f"Conflict error occurred: {str(exc)} at path: {request.url.path}")
        return JSONResponse(
            status_code=409,
            content={
                "error": {
    ...
    async def general_exception_handler(request, exc: Exception):
        logger.error(f"Unexpected error occurred: {str(exc)} at path: {request.url.path}")
        return JSONResponse(
            status_code=500,
            content={
                "error": {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--host", 
        type=str, 
        help="Override host from environment"
    )
    parser.add_argument(
        "--port", 
        type=int, 
        help="Override port from environment"
    )
    parser.add_argument(
        "--no-reload", 
        action="store_true", 
    ...
    if args.seed:
        ...
        try:
            ...
            if seed_path.exists():
                result = subprocess.run([sys.executable, str(seed_path)], 
                                      check=True, 
                                      capture_output=True, 
                                      text=True)
