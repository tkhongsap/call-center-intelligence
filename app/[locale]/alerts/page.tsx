...
function getSpikeTimeWindow(alert: Alert): string | undefined {
  ...
  const timeWindowPatterns = [
    ...
    {
      pattern: /(\d+) hours?/i,
      label: (match: RegExpMatchArray) => `Last ${match[1]} hours`,
    },
  ];

  for (const { pattern, label } of timeWindowPatterns) {
    const match =
      alert.title.match(pattern) || alert.description.match(pattern);
    if (match) {
...
function AlertList({
  ...
  pagination,
  emptyText,
}: AlertsResponse & { emptyText: string }) {
  if (alerts.length === 0) {
    return (
  ...
  return (
    <>
      ...
      <div className="flex-1 p-4 md:p-6 overflow-auto">
        <Suspense
          fallback={
            <div className="h-20 bg-slate-100 rounded-lg animate-pulse mb-4" />
          }
        >
          <AlertFilters />
        </Suspense>
