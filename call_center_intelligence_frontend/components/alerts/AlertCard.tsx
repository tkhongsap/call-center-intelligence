...
import type { Alert, Case } from "@/lib/types";
...
function AlertStats({ alert }: { alert: Alert }) {
  if (alert.baseline_value === null || alert.current_value === null) {
    return null;
  }

  const increase =
    alert.percentage_change !== null && alert.percentage_change > 0;
  ...
  return (
    <div className="flex items-center gap-4 p-3 bg-slate-50 rounded-lg">
      ...
      <div className="flex items-center text-slate-400">
        <ArrowUpRight
          className={`w-5 h-5 ${increase ? "text-red-500" : "text-green-500 rotate-90"}`}
        />
      </div>
      <div className="flex-1">
...
export function AlertCard({
  ...
  const handleShare = async (recipientId: string, message: string) => {
    if (onShare) {
      await onShare(
        ...
        recipientId,
        message,
        shareType === "escalate" ? "escalation" : "share",
      );
    }
  };
  ...
  return (
    <>
      <div
        className={`bg-white rounded-lg border border-slate-200 ${styles.border} overflow-hidden hover:shadow-md transition-shadow`}
      >
        {/* Header */}
        <div
          className={`px-4 py-3 ${styles.headerBg} border-b border-slate-200`}
        >
            ...
            <div className="flex items-start sm:items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${statusStyles}`}
                  >
                    {alert.status}
                  </span>
                </div>
                <div className="flex flex-wrap items-center gap-2 mt-0.5">
                  ...
                  {alert.business_unit && (
                    <Badge variant="default">{alert.business_unit}</Badge>
                  )}
                </div>
              </div>
            </div>
