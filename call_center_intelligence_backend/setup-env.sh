...
# Note: The main .env file is now located in the project root
...
if [ ! -f ../.env ]; then
    echo "Creating .env file in project root from .env.example..."
    cp ../.env.example ../.env
...
if [ "$1" = "production" ]; then
    ...
    echo ""
    echo "Current .env file in project root is ready for development by default."
fi

echo ""
echo "Environment setup complete!"
echo "You can now run: make dev (from project root)"